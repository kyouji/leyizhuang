        ...
        <div class="pagebox" style="float: left; padding: 15px 0;">
                    ...
                    <#if consult_page.totalPages gt 0> 
                        <#list 1..consult_page.totalPages as page> 
                            <#if page <= 3 || (consult_page.totalPages-page) < 3||(consult_page.number+1-page)?abs<3 > 
                                ...
                                <#else>
                                    <a href="javascript:getConsult(${goodsId},(${page!'0'})-1)">${page}</a>
                                <#-- ${page} --> 
                                </#if> 
                                <#assign continueEnter=false> 
                            <#else> 
                                <#if !continueEnter> <span> ...</span> 
            ...
            <div class="page">
                <input class="sub" type="submit" value="确定"
                    onclick="javascript:btnPageSubmit('consult');"
                    style="border: none;" /> <span>页</span> <#if consult_page??> <input
                    class="text" type="text" value="${consult_page.number+1}"
                    id="consultPageNum" /> <span>到第</span> <#else> <input class="text"
                    type="text" value="0" id="consultPageNum" /> <span>到第</span>
                </#if>
            </div>
            <div class="clear"></div>
        </div>
