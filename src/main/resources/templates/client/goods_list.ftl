		...
		<div class="c_l">
								...
								<dd class="select_cont_brand">
									...
									<#if param.valueList??> 
										<#list param.valueList?split(",") as value> 
											<#if value!=""> 
												<a href="${categoryId!'0'}-${brandIndex!'0'}<#list param_index_list as pindex><#if param_index==pindex_index>-${value_index+1}<#else>-${pindex!'0'}</#if></#list>-${orderId!'0'}-${soldId!'0'}-${priceId!'0'}-${timeId!'0'}-${pageId!'0'}-${leftId!'0'}<#if priceLow?? && priceHigh??>_${priceLow?string("#.##")}-${priceHigh?string("#.##")}</#if>" 
													<#if param_index_list[param_index]==value_index+1>class="select_cont_choiced"</#if>>
													${value?trim!""}
												</a>
											</#if> 
										</#list> 
			...
			<#-- 参数结束 -->
				...
				<#if param_list?? && param_list?size gt 4>
					<div class="choose_more">
						<a href="javascript:chooseMoreDown();">
							下拉，更多选项<img src="/client/images/arrow01.png" />
						</a>
					</div>
				</#if>
			</div>
			...
			<div class="sort">
				...
				<div class="sort_page">
					...
					<span>
						<#if goods_page.totalPages==0>
							0
						<#else>
							${goods_page.number+1}
						</#if>
							/${goods_page.totalPages!"0"}
					</span>
				...
				<#if goods_page?? && goods_page.content?size gt 0> 
					<#list goods_page.content as item>
						<dl>
							...
							<dd class="money16">￥
								<#if item.salePrice??>
									${item.salePrice?string("0.00")}
								</#if> 
								<span class="moneydelete">￥
									<#if item.marketPrice??>
										${item.marketPrice?string("0.00")}
									</#if>
								</span>
							</dd>
							<dd class="piclist_ie">
				...
				<#list most_sold_list as item> 
			 		<#if item_index < 6 >
						<dl>
							<dt>
								<a href="/goods/${item.id?c}">
									<img src="${item.coverImageUri!''}" width="210px" height="210px" />
								</a>
							</dt>
							<dd class="c_r_title">
								<a href="/goods/${item.id}" title="${item.title!''}">${item.title!''}</a>
							...
							<dd class="money16">￥
								<#if item.salePrice??>${item.salePrice?string("0.00")}</#if> 
								<span class="moneydelete">￥
									<#if item.marketPrice??>${item.marketPrice?string("0.00")}</#if>
								</span>
							</dd>
						</dl>
					</#if>
