...
function changePage(pageId){
    if(null == pageId){
        var pageId = document.getElementById();
    }
    var nowPage = pageId;
}
</script>
...
<div class="pagebox" style="float:left; padding:15px 0;">
    <div class="num">
        ...
        <#else>
            <a href="javascript:changePage(${(pageId!0)-1});">上一页</a>
        </#if>
            ...
            <#list 1..tuan_goods_page.totalPages as page>
                <#if page <= 3 || (tuan_goods_page.totalPages-page) < 3 || (tuan_goods_page.number+1-page)?abs<3 >
                    <#if page == tuan_goods_page.number+1>
                        <a class="sel" href="javascript:;"">${pageId}</a>
                    <#else>
                        <a href="javascript:changePage(${pageId-1});">${pageId}</a>
                    </#if>
                    <#assign continueEnter=false>
                <#else>
                    <#if !continueEnter>
        ...
        <#else>
            <a class="a2" href="javascript:changePage(${(pageId!0)+1});">下一页</a>
        </#if>
    ...
    <div class="page">
        ...
        <span>页</span>
        <input id="now_tuan_page" class="text" type="text" value="${(pageId!0)+1}">
        <span>到第</span>
    </div>
    <div class="clear"></div>
</div>
