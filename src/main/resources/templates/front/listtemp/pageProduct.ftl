...
<#if product_list??>
    <#list product_list as product>
      <ul>
        <li><a href="/product/${product.id}" target="_blank"> <img src="<#if product.coverImageUri??>${product.coverImageUri}</#if>" />
          <p><#if product.name??>${product.name}</#if></p>
          <b>￥<#if product.flashSalePrice??>${product.priceMinimum?c}</#if></b> </a>
          <p>已有0人关注</p>  
