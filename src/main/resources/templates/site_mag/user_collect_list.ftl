...
<form name="form1" method="post" action="/admin/user/collect/list?userId=${userId!""}" id="form1">
<div>
...
<div class="location">
  ...
  <i class="arrow"></i>
  <a href="/admin/user/list"><span>用户列表</span></a>
  <i class="arrow"></i>
  <span>关注商品</span>
</div>
<!--/导航栏-->
...
<div class="pagelist">
  ...
  <div id="PageContent" class="default"><span>共<#if user_collect_page??>${user_collect_page.totalElements}</#if>数据,分<#if user_collect_page??>${user_collect_page.totalPages}</#if>页显示,当前为第<#if user_collect_page??>${user_collect_page.number+1}</#if>页</span>
        <#if user_collect_page??>
            ...
            <#else>
                <a href="/admin/user/collect/list?keyword=${keyword!""}&page=${user_collect_page.number-1}&size=${size!""}">&lt;&lt;上一页</a>
            </#if>
            ...
            <#list 1..user_collect_page.totalPages as page>
                <#if page <= 3 || (user_collect_page.totalPages-page) < 3 || (user_collect_page.number+1-page)?abs<3 >
                    ...
                    <#else>
                        <a href="/admin/user/collect/list?keyword=${keyword!""}&page=${page-1}&size=${size!""}">${page}</a>
                    </#if>
                    <#assign continueEnter=false>
                <#else>
                    <#if !continueEnter>
            ...
            <#else>
                <a href="/admin/user/collect/list?recent=${keyword!""}&page=${user_collect_page.number+1}&size=${size!""}">下一页&gt;&gt;</a>
            </#if>
        </#if>
  </div>
</div>
